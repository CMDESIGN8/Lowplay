...
const Missions = () => {
  ...
  useEffect(() => {
    ...
    const fetchMissions = async () => {
      try {
        const response = await api.get('https://lowplay-1.onrender.com/misiones', {
  ...
  const handleCompleteMission = async (misionId) => {
    ...
    try {
      const response = await api.post(
        'https://lowplay-1.onrender.com/misiones/completar',
        { misionId },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  return (
    <div className="missions-container">
      <div className="mission-section">
        {missions.length === 0 ? (
          <h4>🎉 Misiones completadas 🎉<br /> ¡Volvé mañana para hacer más misiones y ganar LOWCOINS!</h4>
        ) : (
